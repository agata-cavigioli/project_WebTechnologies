...
<div class="button-div container" style=" width: 40vmin; right:0vw;">
  <div class='row'>
  <button aria-label="Logout" class="signup-button col col-xs-6 p-0" on:click={() => {loggedIn.update(b => !b); userID.set(-1);}}>

      Logout

  </button>
  <div class='col col-xs-6'>
  <Link aria-label="Area Personale" to="/personal" type="submit" style="color:black;" class='text-center'>
...
-->
  ...
  <button aria-label="Signup" class="signup-button" on:click={() => openmodal('Signup')}>Signup</button>
  <button aria-label="Login" class="login-button" on:click={() => openmodal('Login')}>Login</button>
  </div>
{/if}
